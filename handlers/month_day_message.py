...
@router.callback_query(F.data == '30_day')
async def get_one_message(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(Fsm_30.text)


@router.message(Fsm_30.text)
async def get_text_1(mess: Message, state: FSMContext):
    try:
        ...
        await state.set_state(Fsm_30.picture)
    except Exception as e:
        await mess.answer('Что то пошло не так, начните заново')
        await state.clear()


@router.message(Fsm_30.picture)
async def get_picture_1(mess: Message, state: FSMContext):
    ...
    await state.set_state(Fsm_30.go_one)


@router.callback_query(Fsm_30.go_one)
